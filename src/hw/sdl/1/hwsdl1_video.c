...
static void video_setpal_gl_24bpp(const uint8_t *pal, int f, int num)
{
    for (int i = 0; i < num; ++i) {
        for (int j = 0; j < 3; ++j) {
            ...
            video.ppal.p8[(f + i) * 3 + 0] = palette_6bit_to_8bit(pal[i * 3 + 2]);
            video.ppal.p8[(f + i) * 3 + 1] = palette_6bit_to_8bit(pal[i * 3 + 1]);
            video.ppal.p8[(f + i) * 3 + 2] = palette_6bit_to_8bit(pal[i * 3 + 0]);
#else
            video.ppal.p8[(f + i) * 3 + 0] = palette_6bit_to_8bit(pal[i * 3 + 0]);
            video.ppal.p8[(f + i) * 3 + 1] = palette_6bit_to_8bit(pal[i * 3 + 1]);
            video.ppal.p8[(f + i) * 3 + 2] = palette_6bit_to_8bit(pal[i * 3 + 2]);
#endif
        }
    }
    hw_video_refresh(1);
