...
static struct sdl_video_s {
    ...
    */
    ...
    /* These are (1) the 320x200x8 paletted buffer that we copy the active buffer to,
       (2) the 320x200 intermediate buffer of the window/renderer's pixel format that
       we blit the former buffer to,
       (3) the intermediate 320x200 texture that we update with the intermediate's buffer data,
...
static void video_create_upscaled_texture(bool force)
{
    ...
    */
    ...
    if (!hw_opt_two_step_scaling || ((w % video.bufw == 0) && (h % video.bufh == 0))) {
...
static void video_adjust_window_size(int *wptr, int *hptr)
{
    ...
    if (hw_opt_int_scaling) {
        int scale = MIN(w / video.bufw, h / video.actualh);
        w = video.bufw * scale;
        h = video.actualh * scale;
    } else if ((w * video.actualh) <= (h * video.bufw)) {
...
static void video_update(void)
{
    ...
    /* Blit from the paletted 8-bit screen buffer to the intermediate
       buffer that we can load into the texture.
    */
    ...
    if (hw_opt_two_step_scaling && (video.texture_upscaled != NULL)) {
...
static int video_sw_set(int w, int h)
{
    ...
    */
    ...
    /* Format of interbuffer must match the screen pixel format because we
       copy the surface data into the texture.
    */
