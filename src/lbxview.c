...
static void drawscreen_outlbx(void)
{
    ...
    for (lbxfile_e i = 0; i < LBXFILE_NUM; ++i) {
        font8x8_drawstr(0, 0, " #  Filename    Type Items", textcolor, 0);
        sprintf(linebuf, "%x", i);
        font8x8_drawstr(1 * 8, 8 + ((int)i) * 8, linebuf, textcolor, 0);
        font8x8_drawstr(4 * 8, 8 + ((int)i) * 8, lbxfile_name(i), textcolor, 0);
        sprintf(linebuf, "%u  %u", lbxfile_type(i), lbxfile_num_items(i));
        font8x8_drawstr(18 * 8, 8 + ((int)i) * 8, linebuf, textcolor, 0);
        font8x8_drawchar(0, (cursor_i + 1) * 8, ' ', 0, textcolor);
    }
    for (int k = 0; k < 16; ++k) {
        uint8_t *p, *q;
    ...
    font8x8_drawstr(0, 320, linebuf, textcolor, 0);
}
...
static void drawscreen_inlbx_rotate(void)
{
    ...
    if ((maxwh < 80) && (test_rotate == 1)) {
        ...
        for (int i = 0; i < 8; ++i) {
            ...
            x1 = midx + xo + x / 2;
            y1 = midy + yo + y / 2;
            gfx_aux_draw_frame_from_rotate_limit(x0, y0, x1, y1, &gfxaux);
            angle = (angle + 45) % 360;
        }
    } else {
...
static void drawscreen_inlbx_data(const uint8_t *p, int len)
{
    ...
    font8x8_drawstr(0, 200 + 8 * 2, linebuf, textcolor, 0);
    SETMIN(len, 40 * 25);
    font8x8_drawstrlen(0, 0, (const char *)&p[pos], len, textcolor, 0);
}
...
static void drawscreen_inlbx(void)
{
    ...
    font8x8_drawstr(8, 200 + 8 * 4, "#  Name", textcolor, 0);
    for (int i = 0; i < 20; ++i) {
        int j = i + cursor_offs;
        if (j >= cur_items) {
            break;
        ...
        linebuf[3 + 32] = 0;
        font8x8_drawstr(8, 200 + 8 * (5 + i), linebuf, textcolor, 0);
        font8x8_drawchar(0, 200 + (cursor_i - cursor_offs + 5) * 8, ' ', 0, textcolor);
    }
    {
        uint32_t offs, len;
        ...
        font8x8_drawstr(0, 200 + 8 * 0, linebuf, textcolor, 0);
    }
    if (lbxfile_type(cur_lbx) == LBX_TYPE_GFX) {
        ...
        if (test_rotate) {
            ...
            gfx_aux_draw_frame_from_limit(cur_xoff, cur_yoff, &gfxaux);
        }
        if (advance_frame) {
            advance_frame = false;
        } else {
