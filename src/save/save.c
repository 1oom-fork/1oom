...
uint8_t *libsave_get_savebuf(void)
{
    return savebuf;
}
...
const char *libsave_select_year_fname(int year)
{
    const char *path = os_get_path_user();
    char namebuf[32];
    int res;
...
{
    int res;
    const char *filename = libsave_select_year_fname(year);
...
int game_save_do_save_year(const char *savename, const struct game_s *g)
{
    ...
    filename = libsave_select_year_fname(g->year + YEAR_BASE);
    if (!savename) {
