...
int game_save_do_load_do(const char *filename, struct game_s *g, int savei)
{
    FILE *fd = NULL;
    uint8_t *savebuf;
    int res = -1, len = 0;
    ...
    || fseek(fd, GAME_SAVE_HDR_SIZE, SEEK_SET)) {
...
bool game_save_check_header(const char *filename, int i)
{
    uint8_t hdr[GAME_SAVE_HDR_SIZE];
    FILE *fd;
