...
static void main_menu_make_game_custom_page(struct main_menu_data_s *d)
{
    ...
    menu_make_enum(menu_allocate_item(), "AI ID", mm_get_custom_ai_id_value, &game_opt_custom.ai_id, 0, GAME_AI_NUM_VISIBLE - 1, MOO_KEY_a);
...
static void main_menu_make_options_addons_page(struct main_menu_data_s *d)
{
    ...
    menu_make_bool(menu_allocate_item(), "UI Governor", &ui_governor_enabled, MOO_KEY_g);
    menu_make_bool(menu_allocate_item(), "Combat Autoresolve", &ui_space_combat_autoresolve, MOO_KEY_v);
    menu_make_bool(menu_allocate_item(), "UI Xtramenu", &ui_xtramenu_enabled, MOO_KEY_x);
    menu_make_bool(menu_allocate_item(), "UI Msg Filter", &ui_msg_filter_enabled, MOO_KEY_m);
