...
static void main_menu_draw_cb(void *vptr)
{
    ...
    if (force_restart) {
        ...
        lbxfont_print_str_center(160, 60, "Application restart required", UI_SCREEN_W);
    }
    for (int i = 0; i < d->item_count; ++i) {
        ...
        lbxfont_print_str_center(it->x, it->y, buf, UI_SCREEN_W);
    }
...
static void main_menu_make_options_rules_orbital_bombardment_page(struct main_menu_data_s *d)
{
    ...
    menu_make_bool(menu_allocate_item(), "Allow Any Weapon", &game_fix_orbital_weap_any, MOO_KEY_UNKNOWN);
    menu_make_bool(menu_allocate_item(), "Allow Weapon 4", &game_fix_orbital_weap_4, MOO_KEY_UNKNOWN);
    menu_make_bool(menu_allocate_item(), "Torpedo Damage Fix", &game_fix_orbital_torpedo, MOO_KEY_UNKNOWN);
    menu_make_bool(menu_allocate_item(), "Computer Bonus Fix", &game_fix_orbital_comp, MOO_KEY_UNKNOWN);
    menu_make_back(menu_allocate_item());
}
