...
static void save_game_draw_cb(void *vptr)
{
    ...
    lbxgfx_draw_frame(0, 0, d->gfx_savegame);
    for (int i = 0; i < NUM_SAVES; ++i) {
        lbxgfx_draw_frame(16 + xoff, 23 + yoff + 18 * i, (d->selected == i) ? d->gfx_lg_green : d->gfx_lg_gray);
    }
}
...
int ui_save_game(struct game_s *g)
{
    ...
    while (!flag_done) {
        ...
        for (int i = 0; i < NUM_SAVES; ++i) {
            ...
            oi_save[i] = uiobj_add_textinput(149, 35 + i * 18, 106, &(d.tbl_savename[i][0]), SAVE_NAME_LEN - 1, 1, false, 0, ctbl, MOO_KEY_UNKNOWN, -1);
