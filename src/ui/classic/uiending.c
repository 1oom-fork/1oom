...
static void ui_play_winlose_cb1(void *vptr)
{
    ...
    int f = p->frame;
    vgabuf_erase();
    lbxgfx_draw_frame_offs(0x000 - f, 0, p->gfx_stars);
    lbxgfx_draw_frame_offs(0x140 - f, 0, p->gfx_stars);
    if (f > 0x32) {
        lbxgfx_draw_frame(0, 0, p->gfx_ships);
        lbxgfx_draw_frame_offs(0x13f - ((f - 0x32) * 3) / 2, 0, p->gfx_planets);
    }
...
static void ui_play_ending_good_or_tyrant(int race, const char *name, bool flag_good)
{
    ...
    uiobj_table_clear();
    uiobji_ma = uiobj_add_mousearea_all(MOO_KEY_UNKNOWN, -1);
...
static void ui_play_winlose_exile_cb(void *vptr)
{
    ...
    lbxgfx_draw_frame_offs(0x000 - f, 0, p->gfx_stars);
    lbxgfx_draw_frame_offs(0x140 - f, 0, p->gfx_stars);
    lbxgfx_draw_frame_offs(0x280 - f, 0, p->gfx_stars);
    lbxgfx_draw_frame_offs(f * 3 - 0xf0, 0, p->gfx_ships);
...
static void ui_play_winlose_funeral_cb(void *vptr)
{
    ...
    if ((f > 0x14) && (f < 0xa6)) {
        ...
        lbxgfx_draw_frame_offs(x, y, p->gfx_coffin);
        lbxgfx_draw_frame_offs(x, y, p->gfx_march);
    }
