...
static void planets_draw_cb(void *vptr)
{
    ...
    v = g->eto[d->api].tax;
    if (v > 0) {
        vgabuf_draw_line_3h(213, 160, 212 + (v * 4) / 25, 0x73);
    }

    lbxfont_select(2, 8, 0, 0);
    lbxfont_print_str_normal(210, 170, game_str_pl_reserve);
    lbxfont_select(2, 6, 0, 0);
    v = (e->tax * e->total_production_bc) / 2000;
    sprintf(buf, "+%i", v);
    lbxfont_print_str_right(272, 159, buf);
    lbxfont_print_str_normal(276, 159, game_str_bc);
    lbxfont_print_str_normal(276, 170, game_str_bc);
    lbxfont_print_num_right(272, 170, e->reserve_bc);
    ...
    for (int i = 0; i < PLANETS_ON_SCREEN; ++i) {
        ...
        if (pi < d->num) {
            ...
            lbxfont_print_num_right(17, y0, pi + 1);
            lbxfont_select(2, 0xd, 0, 0);
            lbxfont_print_str_normal(25, y0, p->name);
            lbxfont_select(2, 6, 0, 0);
            lbxfont_print_num_right(83, y0, p->pop);
            if (p->pop != p->pop_prev) {
                int v;
                char c;
                uint8_t *gfx;
                ...
                lbxgfx_draw_frame(92, y0 - 1, gfx);
                sprintf(buf, "%c%i", c, v);
                lbxfont_print_str_right(111, y0, buf);
            }
            lbxfont_print_str_right(149, y0, game_str_tbl_roman[p->shield]);
            /*lbxfont_select(2, 6, 0, 0);*/
            lbxfont_print_num_right(132, y0, p->factories);
            lbxfont_print_num_right(170, y0, p->missile_bases);
            if (p->waste) {
                lbxfont_print_num_right(189, y0, p->waste);
            }
            ...
            lbxfont_print_num_right(214, y0, v);
            str = NULL;
            lbxfont_select(2, 0xb, 0, 0);
            ...
            if (str) {
                lbxfont_print_str_normal(268, y0, str);
            }
            if (p->slider[PLANET_SLIDER_SHIP] > 0) {
                ...
                lbxfont_print_str_normal(221, y0, str);
            }
        }
    }
    ...
    lbxfont_print_str_right(59, 174, buf);
    game_print_prod_of_total(g, d->api, e->bases_maint_bc, buf);
    lbxfont_print_str_right(59, 185, buf);
    ...
    lbxfont_print_str_right(116, 174, buf);
    v = e->security;
    sprintf(buf, "%i.%i%%", v / 10, v % 10);
    lbxfont_print_str_right(116, 185, buf);
    sprintf(buf, "%i %s", e->total_trade_bc, game_str_bc);
    lbxfont_print_str_right(195, 174, buf);
    sprintf(buf, "%i %s", e->total_production_bc, game_str_bc);
    lbxfont_print_str_right(195, 185, buf);

    lbxfont_select_set_12_1(5, 8, 0, 0);
    lbxfont_print_str_center(66, 161, game_str_pl_spending);
    lbxfont_print_str_center(163, 161, game_str_pl_tincome);
}
...
static void planets_transfer_draw_cb(void *vptr)
{
    ...
    lbxfont_print_str_center(x + 57, y + 11, game_str_pl_transof);
    lbxfont_print_str_center(x + 57, y + 20, buf);
    lbxfont_select(2, 6, 0, 0);
    lbxfont_print_str_right(x + 104, y + 35, game_str_bc);
    {
        int v;
        v = (d->amount_trans * e->reserve_bc) / 100;
        lbxfont_print_num_right(x + 95, y + 35, v);
    }
}
