...
static void ui_help_draw(const char *str0, const char *str1, int x, int y, int w, int ltype, int lx1, int ly1, const uint8_t *ctbl, uint8_t c0, uint8_t c1, uint8_t c2, uint8_t c3, uint8_t c4, uint8_t c5, uint8_t c6, int colorpos)
{
    ...
    if (ltype != 0) {
        if ((lx1 >= x) && (lx1 <= x1)) {
            ...
            lx0 = (x + x1) / 2;
            ly0 = (ly1 < y) ? (y - 1) : (y1 + 1);
            vgabuf_draw_line(lx0, ly0 + 1, lx1, ly1 + 1, 0);
            vgabuf_draw_line(lx0, ly0, lx1, ly1 + 1, c6);
        } else {
            int lx0, ly0;
            ly0 = (y + y1) / 2;
            if (lx1 < x) {
                if (ltype == 1) {
                    ...
                    if (v > x) {
                        ...
                        vgabuf_draw_line(v, ly0 + 1, x - 1, ly0 + 1, 0);
                        vgabuf_draw_line(lx1, ly1 + 1, v, ly0 + 1, 0);
                        vgabuf_draw_line(v, ly0, x - 1, ly0, c6);
                        vgabuf_draw_line(lx1, ly1, v, ly0, c6);
                    }
                }
            } else {
                if (ltype == 1) {
                    ...
                    if (v < x1) {
                        ...
                        vgabuf_draw_line(x1 + 1, ly0 + 1, v, ly0 + 1, 0);
                        vgabuf_draw_line(v, ly0 + 1, lx1, ly1 + 1, 0);
                        vgabuf_draw_line(x1 + 1, ly0, v, ly0, c6);
                        vgabuf_draw_line(v, ly0, lx1, ly1, c6);
                    }
                }
            }
        }
    ...
    /*2e3d4*/
    vgabuf_draw_box2(x, y, x1, y1, c0, c1, c2, c3);
    vgabuf_put_pixel(x, y, c4);
    vgabuf_put_pixel(x + 1, y + 1, c5);
    vgabuf_put_pixel(x + 2, y + 1, c5);
    vgabuf_put_pixel(x + 1, y + 2, c5);
    vgabuf_put_pixel(x + 1, y + 3, c5);
    vgabuf_draw_line(x + 1, y1 + 1, x1 + 1, y1 + 1, 0);
    vgabuf_draw_line(x1 + 1, y + 1, x1 + 1, y1, 0);
    if (str0[0] != '\0') {
        lbxfont_select_subcolors_13not1();
        lbxfont_print_str_center(x + w / 2, y + 6, str0);
        lbxfont_select_subcolors_0();
        y += lbxfont_get_height() + 5;
    }
    /*2e4ff*/
    lbxfont_print_str_split(x + 6, y + 6, w - 12, str1, 0);
}
