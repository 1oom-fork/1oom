...
#define SX0(p)  ((p)->x0 * (p)->scale)
#define SY0(p)  ((p)->y0 * (p)->scale)
#define SX1(p)  ((p)->x1 * (p)->scale)
#define SY1(p)  ((p)->y1 * (p)->scale)
...
static int ssmidx(const uiobj_t *p)
{
    return SX0(p) + (SX1(p) - SX0(p)) / 2;
}

static int ssmidy(const uiobj_t *p)
{
    return SY0(p) + (SY1(p) - SY0(p)) / 2;
}
...
static inline bool uiobj_is_at_xy(const uiobj_t *p, int x, int y)
{
    x += uiobj_mouseoff;
    y += uiobj_mouseoff;
    if ((x < SX0(p)) || (x > SX1(p)) || (y < SY0(p)) || (y > SY1(p))) {
        return false;
    }
    return true;
}
...
static void uiobj_handle_t4_sub2(uiobj_t *p, uint16_t len, uint16_t a4, const char *str)
{
    ...
    lbxfont_print_str_normal(p->x0, p->y0, strbuf);
    ui_palette_set_n();
    uiobj_finish_frame();
...
static void uiobj_handle_t6_slider_input(uiobj_t *p)
{
    ...
    if (p->t6.vertical == false) {
        di = mouse_get_x() + uiobj_mouseoff;
        slideroff = ((p->t6.vmax - p->t6.vmin) * (di - SX0(p))) / (SX1(p) - SX0(p));
        if (SX1(p) <= di) {
            sliderval = p->t6.vmax;
        } else if (SX0(p) >= di) {
            sliderval = p->t6.vmin;
        ...
        di = mouse_get_y() + uiobj_mouseoff;
        slideroff = ((p->t6.vmax - p->t6.vmin) * (SY1(p) - di)) / (SY1(p) - SY0(p));
        if (SY1(p) <= di) {
            sliderval = p->t6.vmin;
        } else if (SY0(p) >= di) {
            sliderval = p->t6.vmax;
    ...
    switch (p->type) {
        ...
        case UIOBJ_TYPE_TEXTINPUT:
            if (uiobj_focus_oi != oi) {
                ...
                vgabuf_fill_rect(p->x0, p->y0, p->x1, p->y1, p->t4.rectcolor);
                if (!p->t4.align_right) {
                    lbxfont_print_str_normal(p->x0, p->y0, p->t4.buf);
                } else {
                    lbxfont_print_str_right(p->x1, p->y0, p->t4.buf);
                }
            }
            break;
...
static void uiobj_handle_click(uiobj_id_t oi, bool in_focus)
{
    ...
    switch (p->type) {
        ...
        case UIOBJ_TYPE_SCROLLAREA:
            if (in_focus) {
                *p->tb.xptr = (mouse_get_x() - p->x0) / p->tb.xdiv;
                *p->tb.yptr = (mouse_get_y() - p->y0) / p->tb.ydiv;
            }
            break;
        case UIOBJ_TYPE_SLIDER:
            if (in_focus) {
...
static uiobj_id_t uiobj_kbd_dir_key_dy_list(int diry)
{
    ...
    if (oi > 0) {
        p = &uiobj_tbl[oi];
        mouse_stored_x = ssmidx(p);
        mouse_stored_y = ssmidy(p);
        if ((mouse_get_x() != mouse_stored_x) || (mouse_get_y() != mouse_stored_y)) {
...
static uiobj_id_t uiobj_kbd_dir_key_dxdy(int dirx, int diry, uiobj_id_t oi2, int mx, int my)
{
    ...
    if ((diry != 0) && (dirx == 0)) {
        for (uiobj_id_t i = UIOBJI_1; i < uiobj_table_num; ++i) {
            ...
            p = &uiobj_tbl[i];
            if (uiobj_kbd_dir_obj_ok(p)) {
                dy = (diry < 0) ? (my - ssmidy(p)) : (ssmidy(p) - my);
                dx = ssmidx(p) - mx;
                if ((SX0(p) <= mx) && (SX1(p) >= mx) && ((dx < -6 * p->scale) || (dx > 6 * p->scale))) {
                    dx = 6 * p->scale ;
                }
    ...
    if ((dirx != 0) && (diry == 0)) {
        for (uiobj_id_t i = UIOBJI_1; i < uiobj_table_num; ++i) {
            ...
            p = &uiobj_tbl[i];
            if (uiobj_kbd_dir_obj_ok(p)) {
                dx = (dirx < 0) ? (mx - ssmidx(p)) : (ssmidx(p) - mx);
                dy = ssmidy(p) - my;
                if ((SY0(p) <= my) && (SY1(p) >= my) && ((dy < -6 * p->scale) || (dy > 6 * p->scale))) {
                    dy = 6 * p->scale;
                }
...
static uiobj_id_t uiobj_handle_kbd_find_alt(uiobj_id_t oi, uint32_t key)
{
    ...
    while (1
      ...
      && ((p->type != UIOBJ_TYPE_ALTSTR) || !KBD_MOD_ONLY_ALT(key) || (KBD_GET_KEY(key) != p->key))
    ) {
        ++oi;
        p = &uiobj_tbl[oi];
    }
...
static uint32_t uiobj_handle_kbd(uiobj_id_t *oiptr)
{
    ...
    if (oi < uiobj_table_num) {
        uiobj_t *p = &uiobj_tbl[oi];
        *oiptr = oi;
        if (mouse_onscreen_xy(SX0(p), SY0(p))) {
            mouse_stored_x = ssmidx(p);
            mouse_stored_y = ssmidy(p);
            if (mouse_onscreen_xy(mouse_stored_x, mouse_stored_y)) {
                ui_cursor_update_gfx_i(mouse_stored_x, mouse_stored_y);
...
static uiobj_id_t uiobj_handle_input_sub0(void)
{
    ...
    uiobj_t *p;
    int mx = mouse_get_x(), my = mouse_get_y(), mb;
    uiobj_focus_oi = UIOBJI_ESC;
    ...
    if (mouse_buttons == 0) {
        ...
        if (mb == MOUSE_BUTTON_MASK_RIGHT) {
            ...
            mx = mouse_get_click_x();
            my = mouse_get_click_y();
            oi = UIOBJI_NONE;
            /*key = 0;*/
        ...
        if (uiobj_tbl[uiobj_clicked_oi].type == UIOBJ_TYPE_SLIDER) {
            uiobj_do_callback();
        }
        uiobj_clicked_oi = UIOBJI_NONE;
        if (oi != UIOBJI_NONE) {
...
{
    int x = mouse_get_x(), y = mouse_get_y();
    ui_cursor_update_gfx_i(x, y);
    uiobj_mouseoff = ui_cursor_mouseoff;
    for (uiobj_id_t i = UIOBJI_1; i < uiobj_table_num; ++i) {
        uiobj_t *p = &uiobj_tbl[i];
...
uiobj_id_t uiobj_add_mousearea_limited(int16_t x0, int16_t y0, int16_t x1, int16_t y1, mookey_t key, int16_t helpid)
{
    uiobj_t *p = &uiobj_tbl[uiobj_table_num];
...
uiobj_id_t uiobj_add_mousearea_all(mookey_t key, int16_t helpid)
{
    return uiobj_add_mousearea(VGABUF_RECT, key, helpid);
}
