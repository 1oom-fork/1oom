...
static void uiobj_handle_ta_sub1(int x0, int y0, int x1, int y1, uint16_t subtype, uint8_t *p0p, uint16_t p0v, uint16_t p1, uint16_t p2, uint16_t p3, int scale)
{
    ...
    switch (subtype) {
        ...
        case 7:
            ...
            break;
        case 0xd:
            ui_draw_box_grain(x0, y0, x1, y1, p0v, p1, p3, scale);
            break;
        default:
            break;
    }
...
static void uiobj_handle_click(int i, uint16_t a2)
{
    uiobj_t *p = &uiobj_tbl[i];
    switch (p->type) {
        case 0:
                ...
                if ((q->type != 3) || (p->type == 3)) {
                    if (q->type == 0xa) {
                        if ((p->type == 0xa) && p->ta.z12) {
                            uiobj_handle_click(uiobj_focus_oi, 0);
                        }
                    } else {
                        uiobj_handle_click(uiobj_focus_oi, 0);
                    }
...
bool uiobj_read_str(int x, int y, int w, char *buf, int buflen, uint8_t rcolor, bool alignr, uint16_t z1e, const uint8_t *ctbl)
{
    ...
    uiobj_set_downcount(1);
    {
        int16_t oi = uiobj_add_t4(x, y, w, buf, buflen, rcolor, alignr, z1e, ctbl, MOO_KEY_UNKNOWN);
        uiobj_focus_oi = oi;
        p = &uiobj_tbl[oi];
    }
    fonth = lbxfont_get_height();
