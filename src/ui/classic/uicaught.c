...
static void caught_draw_cb(void *vptr)
{
    ...
    lbxfont_print_str_normal(x + 10, y + 8, game_str_sc_caught); /* FIXME split str to 3 and place using x */
    ...
    for (int i = 0; i < g->players; ++i) {
        if (IN_CONTACT(g, d->api, i)) {
            ...
            lbxfont_print_str_normal(x + 11, y, buf);
            v = g->evn.spies_caught[i][d->api];
            lbxfont_print_num_normal(x + 68, y, v);
            v = g->evn.spies_caught[d->api][i];
            lbxfont_print_num_normal(x + 96, y, v);
            ++n;
            y += 8;
        }
    }
