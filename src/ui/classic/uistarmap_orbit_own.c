...
static void ui_starmap_orbit_own_draw_cb(void *vptr)
{
    ...
    if (g->planet_focus_i[d->api] != d->from) {
        ...
        if (!ui_starmap_orbit_own_in_frange(d)) {
            if (d->sn0.num < NUM_SHIPDESIGNS) { /* WASBUG MOO1 compares to 7, resulting in text below last ship */
    ...
    for (int i = 0; i < d->sn0.num; ++i) {
        ...
        lbxfont_select(0, 0xd, 0, 0);
        {
            int y;
            y = 40 + i * 26;
            lbxfont_print_num_right(258, y, d->ships[st], UI_SCREEN_W);
            if (ui_extra_enabled) {
                y = 24 + i * 26;
                lbxfont_select(0, 0x7, 0, 0);
                lbxfont_print_num_right(258, y, d->sn0.ships[i], UI_SCREEN_W);
                lbxfont_select(0, 0xd, 0, 0);
            }
        }
...
do_accept:
            ...
            if ((!ui_extra_enabled) || BOOLVEC_IS_CLEAR(r->visible, PLAYER_NUM)) {
                ...
                ui_starmap_sn0_setup(&d.sn0, NUM_SHIPDESIGNS, os);
                for (int i = 0; i < NUM_SHIPDESIGNS; ++i) {
