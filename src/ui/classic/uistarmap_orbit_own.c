...
static void ui_starmap_orbit_own_draw_cb(void *vptr)
{
    ...
    ui_starmap_draw_basic(d);
    {
        ...
        lbxgfx_draw_frame_offs_delay(x, y, !(d->anim_delay % STARMAP_ANIM_DELAY), ui_data.gfx.starmap.shipbord, STARMAP_LIMITS, UI_SCREEN_W, starmap_scale);
    }
    ...
    if (g->planet_focus_i[d->api] != d->oo.from) {
        ...
        lbxgfx_draw_frame_offs_delay(x1, y1, !(d->anim_delay % STARMAP_ANIM_DELAY), ui_data.gfx.starmap.planbord, STARMAP_LIMITS, UI_SCREEN_W, starmap_scale);
...
do_accept:
        ...
        if (!flag_done) {
            ...
            if (1
              && ((g->planet_focus_i[active_player] != d.oo.from) || ui_extra_enabled)
