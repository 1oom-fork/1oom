...
static void ui_starmap_orbit_own_draw_cb(void *vptr)
{
    ...
    const shipresearch_t *srd = &(g->srd[d->api]);
    char buf[0x80];

    ui_starmap_draw_basic(d);
    {
        int x, y;
        x = ui_starmap_x_to_screen(pf->x) + 17;
        y = ui_starmap_y_to_screen(pf->y) - 1;
    ...
    if (g->planet_focus_i[d->api] != d->from) {
        ...
        x0 = ui_starmap_x_to_screen(pf->x) + 20;
        ...
        ui_starmap_draw_line_ctbl(x0 + 3, y0 + 1, x1 + 6, y1 + 6, ctbl, 5, ui_data.starmap.line_anim_phase);
        gfx = ui_data.gfx.starmap.smalship[g->eto[d->api].banner];
        lbxgfx_set_frame_0(gfx);
        ui_starmap_draw_frame(x0, y0, gfx);
        if (!ui_starmap_orbit_own_in_frange(d)) {
            if (d->ss.sn0.num < (ui_fix_starmap_oor_msg ? 6 : 7)) {
    ...
    for (int i = 0; i < d->ss.sn0.num; ++i) {
        ...
        lbxfont_print_num_right(258, 40 + i * 26, d->ss.ships[st]);
...
#define UIOBJ_CLEAR_LOCAL() \
    ...
    while (!flag_done) {
        ...
        if (!flag_done) {
            ui_starmap_update_mouse_hover(&d, oi2);
            ...
            for (int i = 0; i < d.ss.sn0.num; ++i) {
                ...
                oi_tbl_n[i] = uiobj_add_t0(265, 35 + i * 26, "", ui_data.gfx.starmap.move_but_n, MOO_KEY_UNKNOWN, -1);
            }
