...
static inline void ui_starmap_draw_starback(uint8_t *gfx, int x, int y)
{
    ...
    lx = 222 * ui_scale - 1;
    ly = 178 * ui_scale - 1;
    for (int yb = y; yb < ly; yb += 200) {
        for (int xb = x; xb < lx; xb += 320) {
            lbxgfx_draw_frame_offs(xb, yb, gfx, STARMAP_LIMITS, UI_SCREEN_W, 1);
        }
    }
}
...
void ui_starmap_draw_starmap(struct starmap_data_s *d)
{
    ...
    if (ui_sm_distance_tooltip && ui_data.ui_main_loop_action == UI_MAIN_LOOP_STARMAP && d->dist_i != PLANET_NONE) {
        ...
        ui_draw_line_limit(px, py, qx, qy, 0x06, 1);
        lbxfont_select(0, 0, 0, 0);
        int l = g->gaux->star_dist[j][d->dist_i];
        lbxfont_print_num_center_limit( (px + qx) / 2, (py + qy) / 2 - 2, l, 6, 6, 221, 177, UI_SCREEN_W, 1);
    }
}
...
static void ui_starmap_clamp_xy(const struct game_s *g, int *x, int *y)
{
    if (!ui_sm_expanded_scroll) {
        ...
        SETRANGE(*x, -54 * ui_scale, g->galaxy_maxx - 54 * ui_scale);
        SETRANGE(*y, -43 * ui_scale, g->galaxy_maxy - 43 * ui_scale);
    }
}
