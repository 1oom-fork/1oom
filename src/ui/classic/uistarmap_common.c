...
void ui_starmap_draw_basic(struct starmap_data_s *d)
{
    ...
    if (BOOLVEC_IS0(p->explored, d->api) && !(ui_extra_enabled && g->gaux->flag_cheat_stars)) {
        ...
        if (OWNER_IS_NOT_KNOWN(g, p, d->api) && !(ui_extra_enabled && g->gaux->flag_cheat_stars)) {
...
void ui_starmap_draw_starmap(struct starmap_data_s *d)
{
    ...
    uiobj_set_limits(STARMAP_LIMITS);
    {
        ...
        lbxgfx_draw_frame_offs(x0, y0, gfx1, UI_SCREEN_W);
        lbxgfx_draw_frame_offs(x0 + 320, y0, gfx1, UI_SCREEN_W);
        lbxgfx_draw_frame_offs(x0, y0 + 200, gfx1, UI_SCREEN_W);
        lbxgfx_draw_frame_offs(x0 + 320, y0 + 200, gfx1, UI_SCREEN_W);
        lbxgfx_draw_frame_offs(x1, y1, gfx2, UI_SCREEN_W);
        lbxgfx_draw_frame_offs(x1 + 320, y1, gfx2, UI_SCREEN_W);
        lbxgfx_draw_frame_offs(x1, y1 + 200, gfx2, UI_SCREEN_W);
        lbxgfx_draw_frame_offs(x1 + 320, y1 + 200, gfx2, UI_SCREEN_W);
    }
...
static void ui_starmap_clamp_xy(const struct game_s *g, int *x, int *y)
{
    if (!ui_sm_expanded_scroll) {
        SETRANGE(*x, 0, g->galaxy_maxx - 108 / starmap_zoom);
        SETRANGE(*y, 0, g->galaxy_maxy - 86 / starmap_zoom);
    } else {
        SETRANGE(*x, -54 / starmap_zoom, g->galaxy_maxx - 54 / starmap_zoom);
        SETRANGE(*y, -43 / starmap_zoom, g->galaxy_maxy - 43 / starmap_zoom);
    }
}
...
void ui_starmap_handle_scrollkeys(struct starmap_data_s *d, int16_t oi)
{
    ...
    if (0
      ...
      || (ui_sm_mouse_scroll && (moo_mouse_y <= 0))) {
        if (yh > 0) {
            yh = 0;
        }
        --yh;
      ...
      || (ui_sm_mouse_scroll && (moo_mouse_y >= UI_SCREEN_H - 1))) {
        if (yh < 0) {
            yh = 0;
        }
        ++yh;
    ...
    if (0
      ...
      || (ui_sm_mouse_scroll && (moo_mouse_x <= 0))) {
        if (xh > 0) {
            xh = 0;
        }
        --xh;
      ...
      || (ui_sm_mouse_scroll && (moo_mouse_x >= UI_SCREEN_W - 1))) {
        if (xh < 0) {
            xh = 0;
        }
        ++xh;
