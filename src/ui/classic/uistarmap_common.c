    ...
    if (explored) {
        ...
        if (p->type == PLANET_TYPE_NOT_HABITABLE) {
            ...
            if (str) {
                ...
                if (show_plus && (p->owner == api) && game_planet_can_terraform(g, p, p->owner, ui_extra_enabled)) {
...
void ui_starmap_draw_starmap(struct starmap_data_s *d)
{
    ...
    if (1
      ...
      && (ui_data.ui_main_loop_action != UI_MAIN_LOOP_ORBIT_EN_SEL)
        ...
        tx = (p->x - x) * 2 + 8;
        ty = (p->y - y) * 2 + 8;
        lbxgfx_draw_frame_offs_delay(tx, ty, !d->anim_delay, ui_data.gfx.starmap.planbord, 6, 6, 221, 177, UI_SCREEN_W);
    }
    if (d->anim_delay == 0) {
...
static void ui_starmap_clamp_xy(const struct game_s *g, int *x, int *y)
{
    if (!ui_sm_expanded_scroll) {
        ...
        SETRANGE(*x, -0x36, g->galaxy_maxx - 0x36);
        SETRANGE(*y, -0x2b, g->galaxy_maxy - 0x2b);
    }
}
