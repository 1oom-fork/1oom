...
void ui_starmap_draw_starmap(struct starmap_data_s *d)
{
    ...
    uiobj_set_limits(STARMAP_LIMITS);
    {
        ...
        x1 = ((-x + 1) / 2) + 6;
        y1 = ((-y + 1) / 2) + 6;
        lbxgfx_draw_frame_offs(x0, y0, ui_data.gfx.starmap.starbak2, UI_SCREEN_W);
        lbxgfx_draw_frame_offs(x0 + 320, y0, ui_data.gfx.starmap.starbak2, UI_SCREEN_W);
        lbxgfx_draw_frame_offs(x0, y0 + 200, ui_data.gfx.starmap.starbak2, UI_SCREEN_W);
        lbxgfx_draw_frame_offs(x0 + 320, y0 + 200, ui_data.gfx.starmap.starbak2, UI_SCREEN_W);
        lbxgfx_draw_frame_offs(x1, y1, ui_data.gfx.starmap.starback, UI_SCREEN_W);
        lbxgfx_draw_frame_offs(x1 + 320, y1, ui_data.gfx.starmap.starback, UI_SCREEN_W);
        lbxgfx_draw_frame_offs(x1, y1 + 200, ui_data.gfx.starmap.starback, UI_SCREEN_W);
        lbxgfx_draw_frame_offs(x1 + 320, y1 + 200, ui_data.gfx.starmap.starback, UI_SCREEN_W);
    }
...
void ui_starmap_handle_scrollkeys(struct starmap_data_s *d, int16_t oi)
{
    ...
    if (0
      ...
      || (ui_extra_enabled && (moouse_y >= UI_SCREEN_H - 1))) {
        if (yh < 0) {
            yh = 0;
        }
        ++yh;
    ...
    if (0
      ...
      || (ui_extra_enabled && (moouse_x >= UI_SCREEN_W - 1))) {
        if (xh < 0) {
            xh = 0;
        }
        ++xh;
...
void ui_starmap_add_oi_bottom_buttons(struct starmap_data_s *d)
{
    ...
    d->oi_races = uiobj_add_mousearea(143, 181, 176, 194, (ui_extra_enabled ? MOO_KEY_a : MOO_KEY_r));
