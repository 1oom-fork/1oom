        ...
        if (p->type == PLANET_TYPE_NOT_HABITABLE) {
            ...
            if (str) {
                ...
                if (show_plus && (p->owner == api) && ((p->max_pop2 + g->eto[api].have_terraform_n) < max_pop) && (max_pop < game_num_max_pop)) {
                    lbxfont_print_str_normal(298, 44, "+", UI_SCREEN_W, ui_scale);
                ...
                lbxfont_print_str_right(305, 45, str, UI_SCREEN_W, ui_scale);
...
static void ui_starmap_draw_sliders_and_prod(struct starmap_data_s *d)
{
    ...
    if (p->buildship == BUILDSHIP_STARGATE) {
        ui_draw_filled_rect(229, 141, 274, 116, 0, ui_scale);
    ...
    {
        int v;
        v = game_planet_get_slider_text(g, p, d->api, PLANET_SLIDER_ECO, buf);
...
void ui_starmap_draw_starmap(struct starmap_data_s *d)
{
    ...
    if (1
      ...
      && (ui_data.ui_main_loop_action != UI_MAIN_LOOP_ORBIT_EN_SEL)
        ...
        tx = (p->x - x) * 2 + 8;
        ty = (p->y - y) * 2 + 8;
        lbxgfx_draw_frame_offs(tx, ty, ui_data.gfx.starmap.planbord, STARMAP_LIMITS, UI_SCREEN_W, 1);
    }
    if (d->anim_delay == 0) {
    ...
    for (int i = 0; i < g->enroute_num; ++i) {
        ...
        if (BOOLVEC_IS1(r->visible, d->api)) {
            ...
            if (g->eto[d->api].have_ia_scanner && (p->owner == d->api) && (r->owner != d->api) && (r->dest == g->planet_focus_i[d->api])) {
                ui_draw_line_limit_ctbl(tx + 5, ty + 2, (p->x - x) * 2 + 14, (p->y - y) * 2 + 14, colortbl_line_enroute, 5, ui_data.starmap.line_anim_phase, 1);
            }
    ...
    for (int pi = 0; pi < g->galaxy_stars; ++pi) {
        ...
        if (BOOLVEC_IS1(p->within_srange, d->api)) {
            ...
            tx = (p->x - x) * 2 + 25;
            if (p->have_stargate) {
                lbxgfx_draw_frame_offs(tx, (p->y - y) * 2 + 14, ui_data.gfx.starmap.stargate2, STARMAP_LIMITS, UI_SCREEN_W, 1);
            }
            ++tx;
