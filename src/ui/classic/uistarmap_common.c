...
static void ui_starmap_draw_planetinfo_do(const struct game_s *g, player_id_t api, planet_id_t planet_i, bool explored, bool show_plus)
{
    ...
    if (explored) {
        ...
        if (ui_qol_starmap_planet_neb && (p->battlebg == 0)) {
            vgabuf_draw_box1(228, 25, 309, 52, 0xd3, 0xd3);
        }
        if (p->type == PLANET_TYPE_NOT_HABITABLE) {
            lbxfont_select(0, 0xe, 0, 0);
            lbxfont_print_str_center(269, 32, game_str_sm_nohabit);
            lbxfont_print_str_center(269, 41, game_str_sm__planets);
        } else {
            const char *str = NULL;
            lbxgfx_draw_frame(229, 27, ui_data.gfx.planets.planet[p->infogfx]);
            lbxfont_select(0, 0xd, 0, 0);
            lbxfont_print_str_right(305, 28, game_str_tbl_sm_pltype[p->type]);
            ...
            if (str) {
                ...
                lbxfont_print_str_right(305, 40, str);
            } else {
                ...
                lbxfont_print_str_right(305, 45, str);
                if ((!show_plus) || (g->eto[api].race != RACE_SILICOID)) {
                    max_pop -= p->waste;
                }
                SETMAX(max_pop, 10);
                lbxfont_print_num_right(x, 45, max_pop);
            }
        }
    } else {
        lbxfont_select(5, 0xe, 0, 0);
        lbxfont_print_str_center(269, show_plus ? 27 : 35, game_str_sm_unexplored);
    }
}
...
static void ui_starmap_draw_range_parsec(struct starmap_data_s *d, int y)
{
    ...
    lbxfont_print_str_center(269, y, buf);
}
...
static void ui_starmap_draw_sliders_and_prod(struct starmap_data_s *d)
{
    ...
    for (planet_slider_i_t i = PLANET_SLIDER_SHIP; i < PLANET_SLIDER_NUM; ++i) {
        vgabuf_fill_rect(227, 81 + 11 * i, 244, 90 + 11 * i, p->slider_lock[i] ? 0x22 : 0);
    }

    lbxgfx_draw_frame(224, 5, ui_data.gfx.starmap.yourplnt);
    lbxfont_select(2, 0xd, 0xe, 0);
    sprintf(buf, "%i \x02(%i)\x01", p->prod_after_maint, p->total_prod);
    lbxfont_print_str_right(x - 1, 72, buf);
    lbxfont_select(2, 0xd, 0, 0);
    lbxfont_print_num_right(265, 61, p->pop);
    lbxfont_print_num_right(x - 1, 61, p->missile_bases);
    ...
    if (p->buildship == BUILDSHIP_STARGATE) {
        ...
        lbxgfx_draw_frame(229, 141, ui_data.gfx.starmap.stargate);
        lbxfont_print_str_center(251, 169, game_str_sm_stargate);
    } else {
        const shipdesign_t *sd = &g->srd[d->api].design[p->buildship];
        uint8_t *gfx = ui_data.gfx.ships[sd->look];
        lbxgfx_set_frame_0(gfx);
        lbxgfx_draw_frame(236, 142, gfx);
        lbxfont_print_str_center(252, 169, sd->name);
    }
    ...
    lbxfont_select(2, 6, 0, 0);
    {
        ...
        if ((prod.vtotal < cost) || (p->buildship == BUILDSHIP_STARGATE)) {
            if (prod.vthis < 1) {
                lbxfont_print_str_right(x, 83, game_str_sm_prodnone);
                lbxfont_select(0, 0xd, 0, 0);
                lbxfont_print_str_right(271, 160, "0");
            } else {
                int num = 0, over;
                ...
                lbxfont_print_str_right(x, 83, buf);
                lbxfont_select(0, 0xd, 0, 0);
                if (p->buildship != BUILDSHIP_STARGATE) {
                    lbxfont_print_str_right(271, 160, "1");
                }
            }
        } else {
            int num;
            ...
            lbxfont_print_str_right(x, 83, buf);
            lbxfont_select(0, 0xd, 0, 0);
            lbxfont_print_num_right(271, 160, num);
        }
    }
    lbxfont_select(2, 6, 0, 0);
    {
        ...
        if (prod.vthis == 0) {
            lbxfont_print_str_right(x, 94, game_str_sm_prodnone);
        } else if (prod.vtotal <= v8) {
            lbxfont_print_str_right(x, 94, game_str_sm_defupg);
        } else {
            prod.vtotal -= v8;
            ...
            SETMAX(va, 0);
            if (prod.vtotal <= va) {
                lbxfont_print_str_right(x, 94, game_str_sm_defshld);
            } else {
                int num, over;
                prod.vtotal -= va;
                SETMAX(prod.vtotal, 0);
                ...
                lbxfont_print_str_right(x, 94, buf);
            }
        }
    }
    {
        ...
        lbxfont_print_str_right(x, 105, str);
    }
    {
        const char *str = NULL;
            ...
            if (vthis > 0) {
                ...
                if (vthis < adjwaste) {
                    ...
                    if (growth <= 0) {
                        ...
                        lbxfont_print_str_right(297, 116, buf);
        ...
        if (str) {
            lbxfont_print_str_right(x, 116, str);
        }
    }
    {
        ...
        lbxfont_print_str_right(x, 127, buf);
    }
}
...
void ui_starmap_draw_basic(struct starmap_data_s *d)
{
    ...
    if (!PLANET_FOCUS_IS_EXPLORED(g, d->api)) {
        lbxgfx_draw_frame(224, 5, ui_data.gfx.starmap.unexplor);
        lbxfont_select_set_12_4(5, 1, 0, 0);
        lbxfont_print_str_split(232, 74, 76, game_str_tbl_sm_stinfo[p->star_type], 2);
        ...
        if (owner == PLAYER_NONE) {
            lbxgfx_draw_frame(224, 5, ui_data.gfx.starmap.no_colny);
            ui_data.gfx.colonies.current = ui_data.gfx.colonies.d[p->type * 2];
            lbxgfx_draw_frame(227, 73, ui_data.gfx.colonies.current);
            ...
            lbxgfx_draw_frame(224, 5, ui_data.gfx.starmap.en_colny);
            ...
            if (BOOLVEC_IS1(p->within_srange, d->api)) {
                ...
                lbxfont_print_str_center(269, 75, game_str_sm_lastrep);
                lbxfont_print_str_center(268, 83, buf);    /* TODO combine with above */
                pop = g->seen[d->api][pi].pop;
                bases = g->seen[d->api][pi].bases;
                range_y = 92;
            }
            lbxfont_select(0, 0xd, 0, 0);
            lbxfont_print_num_right(265, 61, pop);
            lbxfont_print_num_right(310, 61, bases);
...
void ui_starmap_draw_starmap(struct starmap_data_s *d)
{
    ...
    for (int i = 0; i < g->nebula_num; ++i) {
        int tx, ty;
        tx = ui_starmap_x_to_screen(g->nebula_x[i]) + 1;
        ty = ui_starmap_y_to_screen(g->nebula_y[i]) + 1;
        ui_starmap_draw_frame(tx, ty, ui_data.gfx.starmap.nebula[i]);
    }
    if (ui_data.starmap.flag_show_grid) {
        int x0, y0, x1, y1;
        for (y0 = 10; y0 < g->galaxy_maxy; y0 += 50) {
            int ty;
            x0 = ui_starmap_x_to_screen(0);
            x1 = ui_starmap_x_to_screen(g->galaxy_maxx);
            ty = ui_starmap_y_to_screen(y0);
            ui_starmap_draw_line(x0, ty, x1, ty, 4);
        }
        for (x0 = 10; x0 < g->galaxy_maxx; x0 += 50) {
            int tx;
            y0 = ui_starmap_y_to_screen(0);
            y1 = ui_starmap_y_to_screen(g->galaxy_maxy);
            tx = ui_starmap_x_to_screen(x0);
            ui_starmap_draw_line(tx, y0, tx, y1, 4);
        }
    }
    for (planet_id_t pi = PLANET_0; pi < g->galaxy_stars; ++pi) {
        const planet_t *p = &g->planet[pi];
        if ((p->owner == d->api) && (p->reloc != pi)) {
            const planet_t *p2 = &g->planet[p->reloc];
            int x0, y0, x1, y1;
            x0 = ui_starmap_x_to_screen(p->x) + 8;
            x1 = ui_starmap_x_to_screen(p2->x) + 8;
            y0 = ui_starmap_y_to_screen(p->y) + 8;
            y1 = ui_starmap_y_to_screen(p2->y) + 8;
            ui_starmap_draw_line_ctbl(x0, y0, x1, y1, colortbl_line_reloc, 5, ui_data.starmap.line_anim_phase);
        }
    }
    for (planet_id_t pi = PLANET_0; pi < g->galaxy_stars; ++pi) {
        ...
        if (p->look > 0) {
            tx = ui_starmap_x_to_screen(p->x) + 2;
            ty = ui_starmap_y_to_screen(p->y) + 3;
        } else {
            tx = ui_starmap_x_to_screen(p->x) + 5;
            ty = ui_starmap_y_to_screen(p->y) + 5;
        }
        ui_starmap_draw_frame_from(tx, ty, &ui_data.starmap.star_aux);
        if (p->frame == 4) {
        ...
        if (p->owner != PLAYER_NONE) {
            ...
            if (do_print || (pi == g->evn.planet_orion_i)) {
                ...
                ui_starmap_print_str_center(tx, ty, p->name);
    ...
    if (1
      ...
      && (ui_data.ui_main_loop_action != UI_MAIN_LOOP_ORBIT_EN_SEL)
        ...
        const planet_t *p = &g->planet[g->planet_focus_i[d->api]];
        tx = ui_starmap_x_to_screen(p->x) + 2;
        ty = ui_starmap_y_to_screen(p->y) + 2;
        ui_starmap_draw_frame(tx, ty, ui_data.gfx.starmap.planbord);
    }
    ...
    for (fleet_enroute_id_t i = FLEET_ENROUTE_0; i < g->enroute_num; ++i) {
        if (ENROUTE_IS_VISIBLE(g, i, d->api)) {
            ...
            tx = ui_starmap_x_to_screen(r->x) + 2;
            ty = ui_starmap_y_to_screen(r->y) + 2;
            if (p->x < r->x) {
            ...
            if (g->eto[d->api].have_ia_scanner && (p->owner == d->api) && (r->owner != d->api) && (r->dest == g->planet_focus_i[d->api])) {
                ui_starmap_draw_line_ctbl(tx + 5, ty + 2, (p->x - x) * 2 + 14, (p->y - y) * 2 + 14, colortbl_line_red, 5, ui_data.starmap.line_anim_phase);
            }
            ui_starmap_draw_frame(tx, ty, gfx);
        }
    }
    ...
    if (g->evn.crystal.exists && (g->evn.crystal.killer == PLAYER_NONE)) {
        tx = ui_starmap_x_to_screen(g->evn.crystal.x) + 2;
        ty = ui_starmap_y_to_screen(g->evn.crystal.y) + 2;
        ui_starmap_draw_frame(tx, ty, ui_data.gfx.planets.smonster);
        lbxfont_select(2, 8, 0, 0);
        ui_starmap_print_str_center(tx + 2, ty + 5, game_str_sm_crystal);
    }
    if ((g->evn.amoeba.exists != 0) && (g->evn.amoeba.killer == PLAYER_NONE)) {
        tx = ui_starmap_x_to_screen(g->evn.amoeba.x) + 2;
        ty = ui_starmap_y_to_screen(g->evn.amoeba.y) + 2;
        ui_starmap_draw_frame(tx, ty, ui_data.gfx.planets.smonster);
        lbxfont_select(2, 8, 0, 0);
        ui_starmap_print_str_center(tx + 2, ty + 5, game_str_sm_amoeba);
    }
    for (planet_id_t pi = PLANET_0; pi < g->galaxy_stars; ++pi) {
        const planet_t *p = &g->planet[pi];
        if (BOOLVEC_IS1(p->within_srange, d->api)) {
            ...
            tx = (p->x - x) * 2 + 25;
            if (p->have_stargate) {
                ui_starmap_draw_frame(tx, (p->y - y) * 2 + 7, ui_data.gfx.starmap.stargate2);
            }
            ++tx;
...
void ui_starmap_fill_oi_tbls(struct starmap_data_s *d)
{
    ...
    for (planet_id_t i = PLANET_0; i < g->galaxy_stars; ++i) {
        ...
        if (BOOLVEC_IS1(p->within_srange, d->api)) {
            ...
            x0 = ui_starmap_x_to_screen(p->x) + 20;
            y0 = ui_starmap_y_to_screen(p->y) + 2;
    ...
    for (fleet_enroute_id_t i = FLEET_ENROUTE_0; i < g->enroute_num; ++i) {
        if (ENROUTE_IS_VISIBLE(g, i, d->api)) {
            const fleet_enroute_t *r = &(g->enroute[i]);
            int x0 = ui_starmap_x_to_screen(r->x) + 2;
            int y0 = ui_starmap_y_to_screen(r->y) + 2;
...
void ui_starmap_fill_oi_tbl_stars(struct starmap_data_s *d)
{
    ...
    for (planet_id_t i = PLANET_0; i < g->galaxy_stars; ++i) {
        const planet_t *p = &(g->planet[i]);
        int x0, y0;
        x0 = ui_starmap_x_to_screen(p->x) + 2;
        y0 = ui_starmap_y_to_screen(p->y) + 2;
...
void ui_starmap_draw_planetinfo_2(const struct game_s *g, int p1, int p2, planet_id_t planet_i)
{
    ...
    bool explored = true;
    if (0
      || (IS_HUMAN(g, p1) && !PLANET_IS_EXPLORED(g, planet_i, p1))
      || (IS_HUMAN(g, p2) && !PLANET_IS_EXPLORED(g, planet_i, p2))
    ) {
        explored = false;
    }
