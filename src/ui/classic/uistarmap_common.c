    ...
    if (explored || (ui_extra_enabled && g->gaux->flag_cheat_stars)) {
        ...
        if (p->type == PLANET_TYPE_NOT_HABITABLE) {
            ...
            if (str) {
                ...
                if (show_plus && ((p->owner == api) || (ui_extra_enabled && g->gaux->flag_cheat_stars)) && game_planet_can_terraform(g, p, api, ui_extra_enabled)) {
...
void ui_starmap_draw_starmap(struct starmap_data_s *d)
{
    ...
    for (int i = 0; i < g->nebula_num; ++i) {
        ...
        lbxgfx_draw_frame_offs(tx, ty, ui_data.gfx.starmap.nebula[i], 6, 6, 221, 177, UI_SCREEN_W);
    }
    ...
    if (1
      ...
      && (ui_data.ui_main_loop_action != UI_MAIN_LOOP_ORBIT_EN_SEL)
        ...
        tx = (p->x - x) * 2 + 8;
        ty = (p->y - y) * 2 + 8;
        lbxgfx_draw_frame_offs(tx, ty, ui_data.gfx.starmap.planbord, 6, 6, 221, 177, UI_SCREEN_W);
    }
    if (d->anim_delay == 0) {
    ...
    for (int i = 0; i < g->enroute_num; ++i) {
        ...
        if (BOOLVEC_IS1(r->visible, d->api)) {
            ...
            lbxgfx_draw_frame_offs(tx, ty, gfx, 6, 6, 221, 177, UI_SCREEN_W);
        }
    }
    for (int i = 0; i < g->transport_num; ++i) {
        const transport_t *r = &g->transport[i];
    ...
    if (g->evn.crystal.exists && (g->evn.crystal.killer == PLAYER_NONE)) {
        ...
        lbxgfx_draw_frame_offs(tx, ty, ui_data.gfx.planets.smonster, 6, 6, 221, 177, UI_SCREEN_W);
    ...
    for (int pi = 0; pi < g->galaxy_stars; ++pi) {
        ...
        if (BOOLVEC_IS1(p->within_srange, d->api) || BOOLVEC_IS1(g->eto[d->api].orbit[pi].visible, d->api)) {
            ...
            tx = (p->x - x) * 2 + 25;
            if (p->have_stargate) {
                lbxgfx_draw_frame_offs(tx, (p->y - y) * 2 + 7, ui_data.gfx.starmap.stargate2, 6, 6, 221, 177, UI_SCREEN_W);
            }
            ++tx;
    ...
    if (ui_extra_enabled && ui_data.ui_main_loop_action == UI_MAIN_LOOP_STARMAP && d->dist_i != PLANET_NONE) {
