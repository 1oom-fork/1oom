...
static void design_init_ois(struct design_data_s *d)
{
    ...
    d->oi_name = uiobj_add_textinput(214, 151, 56, d->gd->sd.name, SHIP_NAME_LEN - 1, 1, true, 0, colortbl_sd_textinput, MOO_KEY_UNKNOWN, -1);
...
static struct xy_s ui_design_draw_selbox(int xpos, int xoff1, int xoff2, int xoff3, int n, const char *str)
{
    ...
    ui_cursor_erase_back(); /* HACK should not be needed */
    ui_draw_box_grain(x0 + 4, y0 + 4, x0 + xpos + xoff2, y1 + 20, 1, 2, 0x37);
    vgabuf_limits_set(x0, y0, x1, y1);
    lbxgfx_draw_frame_offs(x0, y0, ui_data.gfx.design.pop1_ul);
    vgabuf_limits_set(x1, y0, VGABUF_W - 1, y1);
    lbxgfx_draw_frame_offs(x0 + xpos + xoff3, y0, ui_data.gfx.design.pop1_ur);
    vgabuf_limits_set(x0, y1, x1, VGABUF_H - 1);
    lbxgfx_draw_frame_offs(x0, y1, ui_data.gfx.design.pop1_dl);
    vgabuf_limits_set(x1, y1, VGABUF_W - 1, VGABUF_H - 1);
    lbxgfx_draw_frame_offs(x0 + xpos + xoff3, y1, ui_data.gfx.design.pop1_dr);
...
static void ui_design_sel_jammer(struct design_data_s *d)
{
...

    {
        ...
        for (ship_jammer_t i = SHIP_JAMMER_NONE; i <= havelast; ++i) {
            if (havebuf[i] >= 0) {
                int space2, power, cost2, sizei, len;
                flag_tbl_enable[n] = (havebuf[i] > 0);
                if (i == actjammer) {
