...
static void ui_starmap_enroute_draw_cb(void *vptr)
{
    ...
    if ((r->owner == d->api) || (g->eto[d->api].have_ia_scanner)) {
        ...
        ui_draw_line_limit_ctbl(x0 + 4, y0 + 1, x1 + 6, y1 + 6, colortbl_line_green, 5, ui_data.starmap.line_anim_phase, starmap_scale);
    ...
    for (int i = 0; i < d->en.sn0.num; ++i) {
        ...
        lbxfont_print_num_right(x + 35, y + 19, d->en.sn0.ships[st], UI_SCREEN_W, ui_scale);
