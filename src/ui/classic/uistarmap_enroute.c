...
static void ui_starmap_enroute_draw_cb(void *vptr)
{
    ...
    for (int i = 0; i < d->en.sn0.num; ++i) {
        ...
        lbxfont_print_num_right(x + 35, y + 19, d->en.sn0.ships[i], UI_SCREEN_W);
...
do_accept:
            ui_sound_play_sfx_24();
            /* BUG: Always true, but otherwise results in undefined behavior
             * due to an incorrect ui_main_loop_action value */
