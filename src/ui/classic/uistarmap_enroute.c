...
static inline bool ui_starmap_enroute_in_frange(struct starmap_data_s *d)
{
    ...
    return ((p->within_frange[d->api] == 1) || ((p->within_frange[d->api] == 2) && d->sn0.have_reserve_fuel));
}
...
static void ui_starmap_enroute_draw_cb(void *vptr)
{
    ...
    if ((r->owner == d->api) && (d->can_move != NO_MOVE)) {
    ...
    if ((r->owner == d->api) || (g->eto[d->api].have_ia_scanner)) {
        ...
        if ((r->owner == d->api) && (d->can_move != NO_MOVE) && !ui_starmap_enroute_in_frange(d)) {
    ...
    for (int i = 0; i < d->sn0.num; ++i) {
        ...
        lbxfont_print_num_right(x + 35, y + 19, d->sn0.ships[st], UI_SCREEN_W);
    ...
    if (1
      && (r->owner == d->api)
      && (d->can_move != NO_MOVE)
      && (!ui_starmap_enroute_in_frange(d) || (game_num_retreat_redir_fix && r->retreat && (d->can_move != GOT_HYPERCOMM) && (pto == d->en.pon)))
    ) {
...
void ui_starmap_enroute(struct game_s *g, player_id_t active_player)
{
    ...
    d.can_move = g->eto[active_player].have_hyperspace_comm ? GOT_HYPERCOMM : NO_MOVE;
    d.en.pon = PLANET_NONE;
    for (int i = 0; i < g->galaxy_stars; ++i) {
        const planet_t *p;
        p = &g->planet[i];
        if ((p->x == r->x) && (p->y == r->y)) {
            if (d.can_move == NO_MOVE) {
                d.can_move = ON_PLANET;
            }
            d.en.pon = i;
            break;
        }
    ...
    ui_starmap_sn0_setup(&d.sn0, g->eto[r->owner].shipdesigns_num, r->ships);
    ui_starmap_update_reserve_fuel(g, &d.sn0, r->ships, active_player);
...
do_accept:
        ...
        if (!flag_done) {
            ...
            if ((r->owner == active_player) && (d.can_move != NO_MOVE)) {
                ...
                if (ui_starmap_enroute_in_frange(&d) && ((!game_num_retreat_redir_fix) || (!r->retreat) || (d.can_move == GOT_HYPERCOMM) || (g->planet_focus_i[active_player] != d.en.pon))) {
