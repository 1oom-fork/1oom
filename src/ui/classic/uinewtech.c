...
static void newtech_choose_next_draw_cb(void *vptr)
{
    ...
    uiobj_set_limits(0, y, 319, y + yo - 1);
    lbxgfx_draw_frame_offs(x, y, d->gfx_pulldown_u, UI_SCREEN_W);
    /* WASBUG
    ...
    */
    ...
    lbxgfx_draw_frame_offs(x, y + yo, d->gfx_pulldown_d, UI_SCREEN_W);
...
again:
    if (d->flag_choose_next) {
        ...
        uiobj_table_clear();
        if (!d->nt.frame) {
            oi_ok = uiobj_add_mousearea_all(MOO_KEY_UNKNOWN);
        } else {
        ...
        while (!flag_done) {
            ...
            if ((oi == oi_o1) || (oi == oi_o2)) {
                flag_done = true;
                ui_sound_play_sfx_24();
                game_diplo_esp_frame(g, (oi == oi_o1) ? d->other1 : d->other2, d->nt.v08);
            }
            newtech_draw_cb1(d);
            ui_draw_finish();
    ...
    for (int i = 0; i < g->evn.newtech[pi].num; ++i) {
        ...
        if (d.nt.frame) {
            ...
            for (int j = 0; (j < g->players) && (d.other2 == PLAYER_NONE); ++j) {
                if ((j != pi) && BOOLVEC_IS1(et->contact, j)) {
