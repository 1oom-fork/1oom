...
static void specs_draw_cb1(void *vptr)
{
    ...
    for (shipdesign_id_t si = SHIPDESIGN_0; si < e->shipdesigns_num; ++si) {
        ...
        lbxfont_print_num_right(35, y + 22, srd->shipcount[si]);
        lbxfont_print_str_normal(49, y + 2, sp.name);
        lbxfont_select(2, 0xb, 0, 0);
        lbxfont_print_num_right(86, y + 13, sp.defense);
        lbxfont_print_num_right(86, y + 23, sp.misdefense);
        lbxfont_print_num_right(126, y + 13, sp.complevel);
        lbxfont_print_num_right(126, y + 23, sp.hp);
        lbxfont_print_num_right(163, y + 3, sp.absorb);
        lbxfont_print_num_right(163, y + 13, sp.engine + 1);
        lbxfont_print_num_right(163, y + 23, sp.man);
        lbxfont_select(2, 0xa, 0, 0);
        for (int wi = 0; wi < WEAPON_SLOT_NUM; ++wi) {
            if (sp.wpnn[wi] != 0) {
                char buf1[64];
                char buf2[12];
                lbxfont_print_num_right(176, y + 3 + wi * 7, sp.wpnn[wi]);
                specs_print_weap(sp.wpnt[wi], sp.wpnn[wi], buf1, buf2);
                lbxfont_print_str_normal(180, y + 3 + wi * 7, buf1);
                if (buf2[0] != '\0') {
                    lbxfont_print_str_right(250, y + 3 + wi * 7, buf2);
        ...
        lbxfont_print_str_center(285, y + 2, game_str_tbl_st_specsh[sp.special[0]]);
        if (sp.special[1] != 0) {
            lbxfont_print_str_center(285, y + 9, game_str_tbl_st_specsh[sp.special[1]]);
        }
        if (sp.special[2] != 0) {
            lbxfont_print_str_center(285, y + 16, game_str_tbl_st_specsh[sp.special[2]]);
        }
        lbxfont_select(2, 0xa, 0, 0);
        lbxfont_print_str_normal(262, y + 23, game_str_sp_cost);
        lbxfont_print_str_normal(305, y + 23, game_str_bc);
        lbxfont_print_num_right(295, y + 23, sd->cost);
    }
...
static void specs_before_draw_cb(void *vptr)
{
    ...
    lbxfont_print_str_split(74, 83, 174, game_str_sp_before, 2);
}
...
static void specs_mustscrap_draw_cb(void *vptr)
{
    ...
    lbxfont_print_str_split(117, 58, 90, game_str_sp_only6, 2);
    lbxgfx_set_frame_0(gfx);
    lbxgfx_draw_frame(114, 102, gfx);
    lbxfont_select(0, 0xd, 0, 0);
    lbxfont_print_num_right(143, 119, srd->shipcount[d->scrapi]);
    lbxfont_select(0, 0, 0, 0);
    lbxfont_print_str_split(150, 107, 60, game_str_sp_wantscrap, 0);
}
...
void ui_specs_before(struct game_s *g, player_id_t active_player)
{
    ...
    while (!flag_done) {
        ...
        uiobj_table_clear();
        oi_ma = uiobj_add_mousearea_all(MOO_KEY_UNKNOWN, -1);
        ui_draw_finish();
    ...
    while (!flag_done) {
        ...
        if (!flag_done) {
            ...
            if (ui_qol_no_cancel_via_lmb) {
                ...
                oi_ma = uiobj_add_mousearea_all(MOO_KEY_o, -1);
            }
            ui_draw_finish();
            ui_delay_ticks_or_click(3);
        }
