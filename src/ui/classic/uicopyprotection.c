...
static void ui_copyprotection_draw_cb(void *vptr)
{
    ...
    lbxgfx_draw_frame(208, 23, ui_data.gfx.ships[look_i[d->seed]]);
    lbxfont_select(5, 0, 0, 0);
    lbxfont_print_str_normal(197, 48, "Pages");
    sprintf(buf, "%d-%d", d->page_min, d->page_max);
    lbxfont_print_str_normal(224, 48, buf);
}
...
void ui_copyprotection_lose(struct game_s *g, struct game_end_s *ge)
{
    struct news_s ns;
    int best_ai = 0;
    planet_id_t best_planet_num = 0;
    planet_id_t planet_num[PLAYER_NUM] = {0, 0, 0, 0, 0, 0};
    player_id_t api = 0;
    ...
    for (planet_id_t si = PLANET_0; si < g->galaxy_stars; ++si) {
