...
const char *ui_planet_str(const struct game_s *g, player_id_t api, uint8_t planet_i, char *buf, size_t bufsize)
{
    const planet_t *p = &(g->planet[planet_i]);
...
uint8_t ui_planet_from_param(struct game_s *g, player_id_t api, struct input_token_s *param)
{
    int n = PLANET_NONE;
        ...
        if (p->type == PLANET_TYPE_NOT_HABITABLE) {
            ...
            if (owner == PLAYER_NONE) {
                ...
                if (show_full) {
                    char buf[10];
                    int v;
                    v = game_planet_get_slider_text(g, planet_i, PLANET_SLIDER_SHIP, buf, sizeof(buf));
                    printf("  - Build ");
                    ...
                    game_planet_get_slider_text(g, planet_i, PLANET_SLIDER_DEF, buf, sizeof(buf));
                    printf("  - Slider %c DEF %3i  %s\n", p->slider_lock[PLANET_SLIDER_DEF] ? '*' : ' ', p->slider[PLANET_SLIDER_DEF], buf);
                    game_planet_get_slider_text(g, planet_i, PLANET_SLIDER_IND, buf, sizeof(buf));
                    printf("  - Slider %c IND %3i  %s\n", p->slider_lock[PLANET_SLIDER_IND] ? '*' : ' ', p->slider[PLANET_SLIDER_IND], buf);
                    v = game_planet_get_slider_text_eco(g, planet_i, true, buf, sizeof(buf));
                    ...
                    printf("%s\n", buf);
                    v = game_get_tech_prod(p->prod_after_maint, p->slider[PLANET_SLIDER_TECH], g->eto[p->owner].race, p->special);
...
int ui_cmd_planet_look(struct game_s *g, player_id_t api, struct input_token_s *param, int num_param, void *var)
{
    uint8_t pi;
    if (num_param == 0) {
...
int ui_cmd_planet_go(struct game_s *g, player_id_t api, struct input_token_s *param, int num_param, void *var)
{
...
int ui_cmd_planet_slider(struct game_s *g, player_id_t api, struct input_token_s *param, int num_param, void *var)
{
...
int ui_cmd_planet_slider_lock(struct game_s *g, player_id_t api, struct input_token_s *param, int num_param, void *var)
{
    planet_t *p = &(g->planet[g->planet_focus_i[api]]);
    planet_slider_i_t si = ui_planet_slider_from_param(param);
    if (p->owner != api) {
...
int ui_cmd_planet_build(struct game_s *g, player_id_t api, struct input_token_s *param, int num_param, void *var)
{
    planet_t *p = &(g->planet[g->planet_focus_i[api]]);
    const empiretechorbit_t *e = &(g->eto[api]);
    if (p->owner != api) {
...
int ui_cmd_planet_reloc(struct game_s *g, player_id_t api, struct input_token_s *param, int num_param, void *var)
{
    uint8_t pfi, pti;
    planet_t *pf;
...
int ui_cmd_planet_trans(struct game_s *g, player_id_t api, struct input_token_s *param, int num_param, void *var)
{
    uint8_t pfi, pti;
    planet_t *pf;
    char buf[0x80];
...
int ui_cmd_planet_reserve(struct game_s *g, player_id_t api, struct input_token_s *param, int num_param, void *var)
{
    planet_t *p = &(g->planet[g->planet_focus_i[api]]);
