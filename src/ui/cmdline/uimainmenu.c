...
static bool ui_new_game(struct game_new_options_s *newopts)
{
    ...
    for (int i = 0; i < GAMEOPTS; ++i) {
        static const char* numkey[9] = { "1", "2", "3", "4", "5", "6", "7", "8", "9" };
        struct input_list_s ng_opts_in[12];
        int j;
        for (j = 0; j <  gameopt_descr[i].opts && j < 9; ++j) {
            ng_opts_in[j].value = j;
            ng_opts_in[j].key = numkey[j];
            ng_opts_in[j].str = NULL;
            ng_opts_in[j].display = gameopt_descr[i].opt[j];
        };
        ng_opts_in[j].value = -1;
        ng_opts_in[j].key = "Q";
        ng_opts_in[j].str = NULL;
        ng_opts_in[j].display = "(quit)";
        ++j;
        ng_opts_in[j].value = 0;
        ng_opts_in[j].key = NULL;
        ng_opts_in[j].str = NULL;
