...
bool game_planet_leaving_trans_fix = false;
bool game_extended_reloc_range = false;
...
void game_update_production(struct game_s *g)
{
    ...
    for (int i = 0; i < g->galaxy_stars; ++i) {
        ...
        if (owner != PLAYER_NONE) {
            ...
            int pop = p->pop;
            int v;
            if (game_planet_leaving_trans_fix) {
                int rebels = p->rebels;
...
#if 0   /* BUG? This code was supposed to be responsible for the operation of space scanners on ships */
                if (BOOLVEC_IS0(p->within_srange, pi) && (srange2 > 0)) {
                    ...
                    for (int j = 0; (j < g->enroute_num) && (mindist1 > srange2); ++j) {
                        if (g->enroute[j].owner == pi) {
                            ...
                            dist = (dist + 9) / 10; /* BUG This line was probably added by mistake */
                            if (dist < mindist1) {
...
{
    return game_extended_reloc_range ? (g->planet[planet_i].within_frange[pi] == 1) : (g->planet[planet_i].owner == pi);
}
