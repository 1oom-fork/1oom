...
static void game_turn_diplo_adjust(struct game_s *g)
{
    ...
    if ((g->year & 1) == 0) {
        ...
        int gdiv = g->galaxy_size + 6 - game_difficulty_limited(g);
        for (player_id_t i = PLAYER_0; i < g->players; ++i) {
            tbl_num_pp[i] = 0;
        }
...
static void game_turn_build_ship(struct game_s *g)
{
    ...
    for (int i = 0; i < g->galaxy_stars; ++i) {
        ...
        if (owner != PLAYER_NONE) {
            ...
            prod += p->bc_to_ship;
	    /* BUG? A 1 BC bonus for having the slider > 0? Never heard of that one. 
	     * Anyway, if you change this, don't forget to update game_planet_get_slider_text() */
...
struct game_end_s game_turn_process(struct game_s *g)
{
    ...
    game_spy_build(g);
    if (game_num_hidden_child_labor) {
        game_update_production(g);
    }
