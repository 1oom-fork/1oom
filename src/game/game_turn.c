...
static inline void game_add_planet_to_build_finished(struct game_s *g, planet_id_t pli, player_id_t owner, uint8_t type)
{
...
static bool game_turn_check_end(struct game_s *g, struct game_end_s *ge)
{
    ...
    }
    {
        ...
        if (!human_alive) {
            struct news_s ns;
            player_id_t killer = g->gaux->human_killer;
            if (killer >= g->players) {
