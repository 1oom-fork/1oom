...
bool game_opt_enable_bomb_animation = true;
        ...
        if ((b->wpn[i].n > 0) && (b->wpn[i].numfire > 0) && (b->wpn[i].numshots != 0)) {
            ...
            if ((range >= dist) && ((!w->is_bomb) || (target_i == 0/*planet*/))) {
                ...
                if ((damagerange != 0) || w->is_bio) {
                    /*5755a*/
                    int absorbdiv;
                    if (!bt->autoresolve && (game_opt_enable_bomb_animation || !w->is_bomb)) {
                    ...
                    if (w->is_bomb) {
                        ...
                        if (!bt->autoresolve && game_opt_enable_bomb_animation) {
...
int game_battle_get_absorbdiv(const struct battle_item_s *b, const struct shiptech_weap_s *w)
{
    int v = 1;
    if (b->sbmask & (1 << SHIP_SPECIAL_BOOL_ORACLE)) {
