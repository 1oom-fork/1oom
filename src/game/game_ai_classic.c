...
struct ai_turn_p1_s {
    ...
    planet_id_t tbl_front_planet[PLAYER_NUM];
    uint32_t force_own_sum;
    int planet_en_num;
    int planet_own_num;
    int tbl_planet_own_w[PLANETS_MAX];
    int tbl_planet_en_w[PLANETS_MAX];
    planet_id_t tbl_planet_own_i[PLANETS_MAX];
    planet_id_t tbl_planet_en_i[PLANETS_MAX];
};
...
static void game_ai_classic_turn_p1_send_scout(struct game_s *g, struct ai_turn_p1_s *ait, player_id_t pi)
{
    ...
    planet_id_t tbl_planet_scout[PLANETS_MAX];
    ...
    for (planet_id_t i = PLANET_0; i < g->galaxy_stars; ++i) {
        ...
        for (player_id_t pi2 = PLAYER_0; pi2 < g->players; ++pi2) {
            if ((pi != pi2) && (e->treaty[pi2] != TREATY_ALLIANCE)) {
                for (shipdesign_id_t j = SHIPDESIGN_0; j < g->eto[pi2].shipdesigns_num; ++j) {
    ...
    for (int i = 0; i < num_to_scout; ++i) {
        ...
        if (mini != PLANET_NONE) {
            if (g->enroute_num >= FLEET_ENROUTE_AI_MAX) {
                ...
                shipdesign_id_t shipi;
                shipi = SHIPDESIGN_NONE;
                for (shipdesign_id_t j = SHIPDESIGN_0; j < e->shipdesigns_num; ++j) {
                    if ((e->orbit[mini].ships[j] != 0) && ((shipi == SHIPDESIGN_NONE) || (!hrf[shipi]))) {
                        shipi = j;
                    }
                }
...
static planet_id_t game_ai_classic_turn_p1_front_find_planet(struct game_s *g, struct ai_turn_p1_s *ait, player_id_t pi, int x, int y)
{
    empiretechorbit_t *e = &(g->eto[pi]);
    planet_id_t mini = 0;
    int mindist = 10000;
    ...
    for (planet_id_t i = PLANET_0; i < g->galaxy_stars; ++i) {
        ...
        if (p->owner == pi) {
            ...
            for (shipdesign_id_t j = SHIPDESIGN_0; (j < e->shipdesigns_num) && (defense == 0); ++j) {
...
static shipcount_t game_ai_classic_turn_p1_spawn_colony_ship(struct game_s *g, struct ai_turn_p1_s *ait, player_id_t pi)
{
    ...
    shipdesign_id_t shipi = e->shipi_colony;
...
static void game_turn_fleet_send(struct game_s *g, struct ai_turn_p1_s *ait, player_id_t pi, planet_id_t from, planet_id_t dest)
{
    empiretechorbit_t *e = &(g->eto[pi]);
    fleet_enroute_t *r;
...
static void game_ai_classic_turn_p1_send_colony_ships(struct game_s *g, struct ai_turn_p1_s *ait, player_id_t pi)
{
    ...
    shipdesign_id_t si = e->shipi_colony;
    int range = e->fuel_range * 15, num_planet_colonize = 0;
    planet_id_t tbl_planet_colonize[PLANETS_MAX];
    ...
    for (planet_id_t i = PLANET_0; i < g->galaxy_stars; ++i) {
        ...
        if (owner == PLAYER_NONE) {
            ...
            for (player_id_t pi2 = PLAYER_0; (pi2 < PLAYER_NUM) && !va; ++pi2) {
                ...
                if (pi2 != pi) {
                    for (shipdesign_id_t j = SHIPDESIGN_0; (j < e2->shipdesigns_num) && !va; ++j) {
        ...
        for (int i = 0; (i < ait->planet_en_num) && work_left; ++i) {
            for (int j = 0; j < (ait->planet_en_num - 1); ++j) {
                ...
                if (ait->tbl_planet_en_w[j] < ait->tbl_planet_en_w[j + 1]) {
                    ...
                    { planet_id_t t; t = ait->tbl_planet_en_i[j]; ait->tbl_planet_en_i[j] = ait->tbl_planet_en_i[j + 1]; ait->tbl_planet_en_i[j + 1] = t; }
                    work_left = true;
                }
            }
        }
            ...
            if (1
              ...
              && ((rnd_1_n(100, &g->seed) < 40) || (ait->planet_en_num < 2))
                ...
                for (player_id_t pi2 = PLAYER_0; pi2 < g->players; ++pi2) {
                    if (pi2 != pi) {
                        e2 = &(g->eto[pi2]);
                        for (shipdesign_id_t l = SHIPDESIGN_0; l < e->shipdesigns_num; ++l) { /* BUG? should be e2->shipdesigns_num? */
                            shipcount_t n;
                      ...
                      && (rnd_1_n(100, &g->seed) < 60)
                        ...
                        for (player_id_t pi2 = PLAYER_0; pi2 < g->players; ++pi2) {
                            if (pi2 != pi) {
                                ...
                                e2 = &(g->eto[pi2]);
                                for (shipdesign_id_t k = SHIPDESIGN_0; k < e2->shipdesigns_num; ++k) {
                                    const shipdesign_t *sd = &(g->srd[pi2].design[k]);
                                    weight += e2->orbit[pto].ships[k] * game_num_tbl_hull_w[sd->hull];
                                }
                            }
                        }
...
static void game_ai_classic_turn_p1_send_defend(struct game_s *g, struct ai_turn_p1_s *ait, player_id_t pi)
{
    ...
    planet_id_t tbl_defend[PLANETS_MAX];
    int num_defend = 0;
    for (player_id_t pi2 = PLAYER_0; pi2 < g->players; ++pi2) {
        const empiretechorbit_t *e2 = &(g->eto[pi2]);
        for (shipdesign_id_t i = SHIPDESIGN_0; i < e2->shipdesigns_num; ++i) {
    ...
    for (fleet_enroute_id_t i = FLEET_ENROUTE_0; i < g->enroute_num; ++i) {
        ...
        if ((r->owner != pi) && (g->planet[r->dest].owner == pi)) {
            ...
            for (shipdesign_id_t j = SHIPDESIGN_0; j < e2->shipdesigns_num; ++j) {
                shipcount_t n;
                n = r->ships[j];
                if (n != 0) {
...
static void game_ai_classic_turn_p1_trans_own(struct game_s *g, struct ai_turn_p1_s *ait, player_id_t pi)
{
    ...
    for (planet_id_t i = PLANET_0; i < g->galaxy_stars; ++i) {
        if (BOOLVEC_IS1(tbl_trans_to, i)) {
            ...
            pfrom = PLANET_NONE;
            pt = &(g->planet[i]);
            for (planet_id_t j = PLANET_0; (j < g->galaxy_stars) && (pfrom == PLANET_NONE); ++j) {
                if (BOOLVEC_IS1(tbl_trans_from, j)) {
                    pf = &(g->planet[j]);
...
static void game_ai_classic_turn_p1(struct game_s *g)
{
    ...
    for (player_id_t pi = PLAYER_0; pi < PLAYER_NUM; ++pi) {
        ...
        for (shipdesign_id_t i = e->shipdesigns_num; i < NUM_SHIPDESIGNS; ++i) {
    ...
    for (player_id_t pi = PLAYER_0; pi < g->players; ++pi) {
        ...
        if ((num_planets / 2) < num_developing_planets) {
            for (player_id_t pi2 = PLAYER_0; pi2 < g->players; ++pi2) {
                /* MOO1 implicitly assumes (PLAYER_0 != pi) */
                if (IS_HUMAN(g, pi2) && IN_CONTACT(g, pi2, pi)) {
...
static void game_ai_classic_design_scrap(struct game_s *g, player_id_t pi, shipdesign_id_t shipi)
{
    empiretechorbit_t *e = &(g->eto[pi]);
    shipresearch_t *srd = &(g->srd[pi]);
    shipdesign_id_t si;
    for (shipdesign_id_t i = shipi; i < (NUM_SHIPDESIGNS - 1); ++i) {
...
static void game_ai_classic_design_ship_base(struct game_s *g, struct ai_turn_p2_s *ait, player_id_t pi)
{
    ...
    if (ait->shiptype == 0/*colony*/) {
        ...
        for (ship_special_t i = SHIP_SPECIAL_BATTLE_SCANNER; i <= special; ++i) {
            tbl_have[i] = 0;
        }
    ...
    }
    {
        ...
        int v;
        shield = game_design_build_tbl_fit_shield(g, &ait->gd, tbl_have);
        v = count_havebuf_items(tbl_have, shield);
        v = game_ai_classic_design_ship_get_item(g, v, tbl_chance[hull]);
    ...
    }
    {
        ...
        ship_armor_t armor;
        armor = game_design_build_tbl_fit_armor(g, &ait->gd, tbl_have);
        if ((rnd_1_n(100, &g->seed) <= tbl_chance[hull]) && ((armor & 1) == 1)) {
            /* NOP */
        } else {
            armor &= ~1;
        }
...
static void game_ai_classic_turn_p2_do(struct game_s *g, player_id_t pi)
{
    ...
    for (player_id_t p2 = PLAYER_0; p2 < g->players; ++p2) {
        const empiretechorbit_t *e2 = &(g->eto[p2]);
        const shipresearch_t *srd2 = &(g->srd[p2]);
    ...
    game_update_maint_costs(g);
    e->shipi_colony = SHIPDESIGN_NONE;
    e->shipi_bomber = SHIPDESIGN_NONE;
    for (shipdesign_id_t i = SHIPDESIGN_0; i < e->shipdesigns_num; ++i) {
    ...
    for (shipdesign_id_t i = SHIPDESIGN_0; (i < e->shipdesigns_num) && (num_non0 > 1); ++i) {
        if (0
    ...
    {
        shipdesign_id_t shipi_remove = SHIPDESIGN_NONE;
        if (num_non0 > 4) {
        ...
        if (shipi_remove != SHIPDESIGN_NONE) {
    ...
    while (e->shipdesigns_num < NUM_SHIPDESIGNS) {
        ...
        srd->year[si] = g->year;
        if (e->shipi_colony == SHIPDESIGN_NONE) {
            e->shipi_colony = si;
            ait->shiptype = 0/*colony*/;
        } else if (e->shipi_bomber == SHIPDESIGN_NONE) {
            e->shipi_bomber = si;
    ...
    {
        shipdesign_id_t tbl_i[NUM_SHIPDESIGNS];
        ...
        for (shipdesign_id_t j = SHIPDESIGN_0; j < NUM_SHIPDESIGNS - 1; ++j) {
            for (shipdesign_id_t i = SHIPDESIGN_0; i < NUM_SHIPDESIGNS - 1; ++i) {
                uint16_t y1, y;
                y1 = tbl_year[i + 1];
                y = tbl_year[i];
                if (y1 > y) {
...
static int game_ai_battle_missile_dmg(const struct battle_s *bt, int missile_i)
{
    ...
    if ((m->target == BATTLE_ITEM_PLANET) && (!w->is_bomb) && (w->misstype > 0)) {
        damagediv = 2;
    }
...
static int game_ai_battle_dmgmax(struct battle_s *bt, int itemi)
{
...
#if 1
    if ((b->side + bt->item[BATTLE_ITEM_PLANET].side) == 1) {
    ...
    int v8 = 0;
    if (b->side == SIDE_L) {
        if (bt->item[BATTLE_ITEM_PLANET].side == SIDE_R) {
            flag_planet_opponent = true;
            v8 = bt->item[BATTLE_ITEM_PLANET].absorb;
        }
        ...
        if (bt->item[BATTLE_ITEM_PLANET].side == SIDE_L) {
            flag_planet_opponent = true;
            v8 = bt->item[BATTLE_ITEM_PLANET].absorb;
        }
        ...
        if (b->wpn[i].numshots != 0) {
            if (w->is_bomb) {
                if (flag_planet_opponent) {
                    int dmg;
                    dmg = w->damagemax - bt->item[BATTLE_ITEM_PLANET].absorb;
                    if (dmg > 0) {
...
static int game_ai_battle_dmggive(struct battle_s *bt, int itemi1, int itemi2, int a2)
{
    ...
    int v = 0, num_weap = (itemi1 == BATTLE_ITEM_PLANET) ? 1 : 4, miss_chance_beam, miss_chance_missile, dist, damagediv = 1, dmg;
    ...
    for (int i = 0; i < num_weap; ++i) {
        ...
        if (itemi1 == BATTLE_ITEM_PLANET) {
            range += 40;
        }
        if (1
          && ((!w->is_bomb) || (itemi2 == BATTLE_ITEM_PLANET))
          && (b->wpn[i].numshots != 0)
    ...
    if ((b->warpdis == 1) && ((dist <= 3) || (a2 > 0)) && (itemi2 != BATTLE_ITEM_PLANET) && (bd->unman < bd->man)) {
        v += 100;
    }
    ...
    for (battle_item_id_t i = BATTLE_ITEM_PLANET; i <= bt->items_num; ++i) {
        ...
        if (((b->side + b2->side) == 1) && (b->num > 0)) {
            ...
            if (dmgmany > 0) {
                ...
                if ((w == 1) && (i != BATTLE_ITEM_PLANET)) {
                    w = 2;
                }
            }
            /*5917c*/
            if ((dmgmax <= 0) && (dmggive > 0)) {
                w = (i == BATTLE_ITEM_PLANET) ? 1 : 2;
            }
            if ((itemi == BATTLE_ITEM_PLANET) && (w > 0)) {
                int dist;
                dist = util_math_dist_maxabs(b->sx, b->sy, b2->sx, b2->sy);
                w += (10 - dist) * 750;
            }
            /*59221*/
            if ((i == BATTLE_ITEM_PLANET) && (w == 1)) {
                for (int j = 0; j < WEAPON_SLOT_NUM; ++j) {
            ...
            if (w > maxw) {
                rival = i;
                maxw = w;
                bt->bases_using_mirv = (tbl_shiptech_weap[bt->item[BATTLE_ITEM_PLANET].wpn[0].t].nummiss > 1);
            }
        }
    }
    if (itemi == BATTLE_ITEM_PLANET) {
...
static int game_battle_ai_best_range(struct battle_s *bt, int target_i)
{
    ...
    if (target_i == BATTLE_ITEM_PLANET) {
        return 1;
    }
    for (int i = 1; i < 10; ++i) {
        int weight;
        ...
        for (int j = 0; j < WEAPON_SLOT_NUM; ++j) {
            ...
            range = (itemi == BATTLE_ITEM_PLANET) ? 12 : 0;
...
static bool game_ai_classic_battle_ai_retreat(struct battle_s *bt)
{
    ...
    if (bt->item[BATTLE_ITEM_PLANET].num > 0) {
        ...
        if (j > BATTLE_ITEM_PLANET) {
            dmg[s] += (game_ai_battle_dmggive(bt, BATTLE_ITEM_PLANET, j, 1) * b->num * 7) / 10;
            dmg[s] -= (repair[j] * 7) / 10;
            repair[j] = 0;
        }
    }
    ...
    }
    {
        ...
        switch (bt->item[BATTLE_ITEM_PLANET].side) {
            case SIDE_L:
                v += 10;
                break;
            case SIDE_R:
...
static bool game_ai_classic_bomb(struct game_s *g, player_id_t player, planet_id_t planet, int pop_inbound)
{
    bool flag_do_bomb;
...
static void game_ai_classic_crank_tech(struct game_s *g, planet_id_t planet)
{
...
static void game_ai_classic_crank_ship(struct game_s *g, planet_id_t planet)
{
    planet_t *p = &(g->planet[planet]);
    int v;
    if ((p->owner == PLAYER_NONE) || IS_HUMAN(g, p->owner)) {
                ...
                if (((v + e1->mood_treaty[p2]) > 150) && (e1->treaty[p2] != TREATY_ALLIANCE)) {
                    ...
                    if ((e1->mood_tech[p2] + 80) < v) {
                        ...
                        */
                        ...
                        int num[2];
                        uint8_t tech[2];
                        num[0] = game_spy_sift_useful_techs(g, p1, p2, rnd_1_n(3, &g->seed));
                        field[0] = s.tbl_field[0];
                        tech[0] = s.tbl_tech2[0];
                        num[1] = game_spy_sift_useful_techs(g, p2, p1, rnd_1_n(3, &g->seed));
