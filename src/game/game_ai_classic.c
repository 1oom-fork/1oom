...
static void game_ai_classic_turn_p1_send_scout(struct game_s *g, struct ai_turn_p1_s *ait, player_id_t pi)
{
    ...
    ait->need_conquer = !rnd_0_nm1(8 - game_difficulty_limited(g), &g->seed);
    for (int i = 0; i < g->galaxy_stars; ++i) {
        const planet_t *p = &(g->planet[i]);
...
static bool game_ai_classic_turn_p1_have_colony_ship(struct game_s *g, struct ai_turn_p1_s *ait, player_id_t pi)
{
    ...
    SETRANGE(prod, 1, 500);
    if (game_difficulty_limited(g) < DIFFICULTY_AVERAGE) {
        if (rnd_0_nm1(6, &g->seed) > game_difficulty_limited(g)) {
            prod = 0;
        }
    }
...
static void game_ai_classic_turn_p1_trans_own(struct game_s *g, struct ai_turn_p1_s *ait, player_id_t pi)
{
    ...
    for (int i = 0; i < g->galaxy_stars; ++i) {
        ...
        if (p->owner == pi) {
            ...
            if (p->pop > ((p->max_pop3 * 3) / 4) && (p->unrest != PLANET_UNREST_REBELLION)) {
...
{
    g->eto[pi].tax = (g->year >= 20) ? (rnd_1_n(10, &g->seed) + game_difficulty_limited(g) + 2) : 0;
}
    ...
    */
    ...
    if (!IS_HUMAN(g, pi)) {
        ...
        if (g->year > 30) {
            ...
            sec += game_difficulty_limited(g) * 8;
            SETMIN(sec, 100);
            e->security = sec;
...
static int game_ai_battle_missile_dmg(const struct battle_s *bt, int missile_i)
{
    ...
    if (bt->g->ai_id == GAME_AI_CLASSIC) {
        int absorbdiv = game_battle_get_absorbdiv(b, w); /* WASBUG checks target's Oracle */
...
static int game_ai_battle_dmggive(struct battle_s *bt, int itemi1, int itemi2, int a2)
{
    ...
    for (int i = 0; i < num_weap; ++i) {
        ...
        int absorbdiv, range;
        if (bt->g->ai_id == GAME_AI_CLASSIC) {
            absorbdiv = game_battle_get_absorbdiv(bd, w); /* WASBUG checks target's oracle */
        } else {
            absorbdiv = game_battle_get_absorbdiv(b, w);
        }
        if (itemi2 == 0/*planet*/) {
...
static int game_battle_ai_missile_evade(const struct battle_s *bt)
{
    ...
    for (int i = 0; (i < bt->num_missile) && (evade <= 1); ++i) {
        ...
        if (m->target == itemi) {
            ...
            dist += movex * 32 - 18;
            if ((rnd_1_n(3, &bt->g->seed) < game_difficulty_limited(bt->g)) && (dist > dangerdist)) {
                evade = 2;
            }
        }
    }
...
static bool game_ai_classic_battle_ai_retreat(struct battle_s *bt)
{
    ...
    }
    {
        int v;
        int64_t w[2];
        v = (5 - game_difficulty_limited(bt->g)) * 5 + 5;
...
{
    uint8_t tech;
    if (rnd_1_n(6, &g->seed) < game_difficulty_limited(g)) {
        tech = tbl[num - 1];
    } else {
...
static void game_ai_classic_diplo_wage_war(struct game_s *g, player_id_t p1, player_id_t p2)
{
    if (g->end != GAME_END_NONE) {
        ...
        if (1
          ...
          && (rnd_1_n(300, &g->seed) <= game_difficulty_limited(g))
        ...
        if (IS_HUMAN(g, p1) && (game_difficulty_limited(g) >= DIFFICULTY_AVERAGE)) {
            int num = 0;
            for (player_id_t p3 = PLAYER_0; p3 < g->players; ++p3) {
            ...
            if (num < game_difficulty_limited(g)) {
...
static void game_ai_classic_turn_diplo_p1(struct game_s *g)
{
    for (player_id_t p1 = PLAYER_0; p1 < g->players; ++p1) {
        ...
        for (player_id_t p2 = PLAYER_0; p2 < g->players; ++p2) {
            ...
            if ((!(rnd_0_nm1(15 - game_difficulty_limited(g) * 2, &g->seed))) && BOOLVEC_IS1(e1->contact, p2)) {
                int v;
